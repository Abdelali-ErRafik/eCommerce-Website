...
{
    public partial class subcategory1 : System.Web.UI.Page
    {
        string cnxStr = ConfigurationManager.ConnectionStrings["EDASHOPDB"].ConnectionString;

        protected void Page_Load(object sender, EventArgs e)
        {
            BindSubCategoryReapater();
        }

        private void BindSubCategoryReapater()
        {
            using (SqlConnection cnx = new SqlConnection(cnxStr))
            {
                SqlCommand cmd = new SqlCommand("select A.*,B.* from SubCategory A inner join Category B on B.CatID = A.MainCatID", cnx);
        ...
        protected void rptBrands_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            if (confirmValue == "Yes")
            {
                if (e.CommandName == "Remove")
                {
                    ...
                    {
                        SqlCommand cmd = new SqlCommand("delete from SubCategory where SubCatID = @subcatID", con);
                ...
                {
                    this.Page.ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Ok Thanks for your fadeback')", true);
                }
            }
        }
        ...
        protected void LnkSearch_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            string qr = "select A.*,B.* from SubCategory A inner join Category B on B.CatID = A.MainCatID where SubCatName like '%" + txtFilter.Text + "%' order by SubCatName asc";
