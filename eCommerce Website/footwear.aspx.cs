...
namespace eCommerce_Website
{
    public partial class footwear : System.Web.UI.Page
    {
        ...
        private void BindProductsReapater()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("procBindAllProductsFootwear", cnx))
                {
        ...
        protected void LnkSearch_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            string qr = "select A.*,B.*,C.BrandName ,A.PPrice-A.PSelPrice as DiscAmount,B.Name as ImageName, C.BrandName as BrandName from Products A inner join Brands C on C.BrandID =A.PBrandID inner join Category as t2 on t2.CatID=A.PCategory cross apply( select top 1 * from ProductImages B where B.PID= A.PID order by B.PID desc )B where t2.CatName='footwear' AND A.PName like '" + txtFilter.Text + "%' order by A.PID desc";
